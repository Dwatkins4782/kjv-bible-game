...
export default function Memorize() {
  ...
  function handleRead() {
    ...
    setReadCount(next)
    if (next >= 3) setStageComplete(true)
  }
  ...
  function checkFirstLetters() {
    ...
    for (let i = 0; i < words.length; i++) {
      if (input[i] && input[i].toLowerCase().replace(/[^a-z]/g, '') === words[i].toLowerCase().replace(/[^a-z]/g, '')) correct++
    }
    setShowResult(true)
  ...
  function checkFullRecall() {
    ...
    if (accuracy >= 0.8) {
      ...
      if (!stats.memorized.includes(selectedVerse.ref)) {
        setStats({ ...stats, memorized: [...stats.memorized, selectedVerse.ref] })
      }
    }
  }
  ...
  if (!selectedCategory) {
    return (
      <div className="fade-in">
        <h2 className="text-gold mb-8">Memorize Scripture</h2>
        <p className="text-muted mb-16">
          {accessibleVerses.length} verses available &bull; Choose a category to begin
        </p>
        <div className="category-grid">
          {categories.map(cat => {
            const catVerses = getVersesByCategory(cat)
            const accessibleCount = catVerses.filter(v => {
              const idx = allVerses.findIndex(av => av.ref === v.ref)
              return !isVerseLocked(idx)
            }).length
            return (
              <div key={cat} className="card card-clickable" onClick={() => setSelectedCategory(cat)}>
                ...
                <p className="text-muted" style={{ fontSize: '0.85rem', marginTop: '4px' }}>
                  {accessibleCount}/{catVerses.length} verses
                </p>
              </div>
            )
          })}
  ...
  if (!selectedVerse) {
    ...
    return (
      <div className="fade-in">
        ...
        {verses.map(v => {
          const idx = allVerses.findIndex(av => av.ref === v.ref)
          const locked = isVerseLocked(idx)
          return (
            <div key={v.ref} className={`card ${locked ? 'verse-locked' : 'card-clickable'}`}
              onClick={() => !locked && startVerse(v)}>
              ...
              <p className="verse-text" style={{ fontSize: '0.95rem', ...(locked ? { filter: 'blur(4px)', userSelect: 'none' } : {}) }}>
        ...
        {tier === 'free' && (
          <div style={{ marginTop: '16px' }}>
            <UpgradePrompt requiredTier="basic" feature="more memory scriptures" />
          </div>
        )}
      </div>
    )
  ...
  // Memorization stages (unchanged logic, added AudioReciteButton)
  const words = selectedVerse.text.split(' ')
  ...
  return (
    <div className="fade-in">
      <div className="flex-between mb-16">
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <AudioReciteButton text={selectedVerse.text} reference={selectedVerse.ref} />
      ...
      <div className="stage-indicator">
        {STAGES.map((s, i) => (
          <div key={i} className={`stage-dot ${i < stage ? 'completed' : i === stage ? 'current' : ''}`} />
        ))}
      </div>
      ...
      <div className="card">
        ...
        {stage === 1 && (
          <>
            ...
            <textarea className="text-input mb-16" rows={4} value={userInput}
              onChange={e => setUserInput(e.target.value)} placeholder="Type the full verse here..." />
            {showResult && (
              <div className="card" style={{
                background: stageComplete ? 'rgba(76,175,80,0.1)' : 'rgba(231,76,60,0.1)',
                borderColor: stageComplete ? 'var(--green)' : 'var(--red)', marginBottom: '12px',
              }}>
                <p>{stageComplete ? 'Well done! You got most of the words right.' : 'Not quite. Try again.'}</p>
                <p className="text-muted mt-16" style={{ fontSize: '0.85rem' }}><strong>Verse:</strong> {selectedVerse.text}</p>
              </div>
            )}
            {!showResult && <button className="btn btn-primary" onClick={checkFirstLetters}>Check</button>}
          ...
          <>
            <div className="verse-text mb-16">
              {words.map((w, i) => {
                ...
                return (
                  <input key={i} type="text" className="text-input"
                    style={{ display: 'inline-block', width: `${Math.max(60, w.length * 12)}px`, padding: '4px 8px', margin: '2px', fontSize: '1rem', textAlign: 'center' }}
                    value={blankAnswers[i] || ''} onChange={e => setBlankAnswers({ ...blankAnswers, [i]: e.target.value })} placeholder="..." />
                )
              })}
            </div>
            {showResult && (
              <div className="card" style={{
                ...
                <p>{stageComplete ? 'Great job!' : 'Some blanks were incorrect. Review and try again.'}</p>
                <p className="text-muted mt-16" style={{ fontSize: '0.85rem' }}><strong>Answers:</strong> {blanks.map(b => `"${b.word}"`).join(', ')}</p>
              </div>
            )}
            {!showResult && <button className="btn btn-primary mt-16" onClick={checkBlanks}>Check</button>}
            {showResult && !stageComplete && (
        ...
        {stage === 3 && (
          <>
            ...
            <textarea className="text-input mb-16" rows={5} value={userInput}
              onChange={e => setUserInput(e.target.value)} placeholder="Type the verse from memory..." />
            {showResult && (
              <div className="card" style={{
                background: stageComplete ? 'rgba(76,175,80,0.1)' : 'rgba(231,76,60,0.1)',
                borderColor: stageComplete ? 'var(--green)' : 'var(--red)', marginBottom: '12px',
            ...
            {!showResult && <button className="btn btn-primary" onClick={checkFullRecall}>Check</button>}
            {showResult && !stageComplete && (
              <button className="btn btn-secondary" onClick={() => { setShowResult(false); setUserInput('') }}>Try Again</button>
            )}
            {showResult && stageComplete && (
